...
/**
 * @title MeluriNFT_Gateway
 * @notice Universal NFT contract on ZetaChain using Gateway directly (no Uniswap dependency)
 * @dev Simplified cross-chain NFT transfers without token swaps
 */
contract MeluriNFT_Gateway is
    Initializable,
    ERC721Upgradeable,
    ...
    ReentrancyGuardUpgradeable
{
    ...
    // Gateway address (mutable for upgradeable pattern)
    ...
    // Modifier to restrict access to gateway only
    ...
    /**
     * @notice Set gateway address (owner only, for upgrades)
     */
    function setGatewayAddress(address gatewayAddress) external onlyOwner {
        if (gatewayAddress == address(0)) revert InvalidAddress();
        gateway = IGatewayZEVM(gatewayAddress);
    }
    ...
    /**
     * @notice Set connected contract for a ZRC20 token
     */
     ...
     * @param destination ZRC20 token representing destination chain
    ...
    function transferCrossChain(
        ...
        // Check if user has enough ZRC20 balance for gas
        uint256 userBalance = IZRC20(destination).balanceOf(msg.sender);
        if (userBalance < gasFee) {
            revert InsufficientGasBalance();
        }
        
        // Transfer ZRC20 tokens from user to contract for gas payment
        ...
        // Revert options (include transferId in revert message)
        ...
        // Approve gateway to spend ZRC20 for gas
    ...
    function onCall(
        ...
        // Verify sender is a connected contract
        if (keccak256(context.sender) != keccak256(connected[zrc20])) {
            revert Unauthorized();
        }

        // Decode message (now includes transferId)
